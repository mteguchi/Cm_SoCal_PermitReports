...
extract_SB_data <- function(run.date, save.files = T){
  save.files <- save.files
  out.file.name <- paste0('data/SB_report_', run.date, '.csv')
  ...
  if (!file.exists(out.file.name)){
    ...
    turtle.haplo.SB <- read_csv(file = out.file.name,
                                col_types = col_def_SB)
    
    
  }
  
  return(turtle.haplo.SB)
}
