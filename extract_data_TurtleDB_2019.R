...
turtle.SDB %>% filter(., Year_caught > 2018) %>% #-> turtle.SDB.CMR
  filter(Month_caught >= 5 & Month_caught < 7) %>%
  mutate(Date = as.Date(paste0(Year_caught, "-", 
...
dat.01 %>% left_join(first.CCL.CMR, by = "ID") -> CMR2019_01_size
...
write_csv(CMR2019_01_size,
          path = paste0('data/CMR2019_size_data_',
                        Sys.Date(), '.csv'))
